    ...
    >
      ...
      <el-form ref="dataFormRef" :model="formData" :rules="rules" label-width="110px">
        <el-row :gutter="16">
          ...
          <el-col :span="12">
            <el-form-item label="限次算法" prop="algorithm">
              <el-select v-model="formData.algorithm" style="width: 100%">
                <el-option
          ...
          <el-col :span="12">
            <el-form-item label="最大次数" prop="maxCount">
              <el-input-number
          ...
          <el-col :span="12">
            <el-form-item label="统计周期(秒)" prop="period">
              <el-input-number
          ...
          <el-col :span="24">
            <el-form-item label="限次维度" prop="dimensions">
              <el-checkbox-group v-model="formData.dimensions">
                <el-checkbox v-for="item in dimensionOptions" :key="item.value" :label="item.value">
                  {{ item.label }}
                </el-checkbox>
              </el-checkbox-group>
            </el-form-item>
          ...
          <el-col :span="24">
            <el-form-item label="Key 前缀" prop="keyPrefix">
              <el-input v-model="formData.keyPrefix" placeholder="ratelimit:login:" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
const algorithmOptions: Array<{ label: string; value: RateLimitAlgorithm }> = [
  { label: "固定窗口计数器", value: "COUNTER" },
  { label: "滑动窗口计数器", value: "SLIDING_WINDOW" },
  { label: "令牌桶", value: "TOKEN_BUCKET" },
  { label: "漏桶", value: "LEAKY_BUCKET" },
];
const dimensionOptions: Array<{ label: string; value: RateLimitDimension }> = [
  { label: "IP 地址", value: "IP" },
  { label: "设备 ID", value: "DEVICE" },
  { label: "用户 ID", value: "USER" },
  { label: "租户 ID", value: "TENANT" },
  { label: "业务 Key", value: "BUSINESS" },
  { label: "自定义", value: "CUSTOM" },
];
const defaultForm = (): RateLimitSceneForm => ({
  sceneCode: "",
  sceneName: "",
  algorithm: "COUNTER",
  dimensions: ["IP"],
...
const rules = reactive<FormRules<RateLimitSceneForm>>({
  sceneCode: [{ required: true, message: "请输入场景编码", trigger: "blur" }],
  sceneName: [{ required: true, message: "请输入场景名称", trigger: "blur" }],
  algorithm: [{ required: true, message: "请选择限次算法", trigger: "change" }],
  dimensions: [
    { type: "array", required: true, min: 1, message: "至少选择一个限次维度", trigger: "change" },
  ],
  maxCount: [{ required: true, message: "请输入最大次数", trigger: "change" }],
  period: [{ required: true, message: "请输入统计周期", trigger: "change" }],
});
const filteredData = computed(() =>
  sceneList.value.filter((item) => {
    ...
    return (
      ...
      (queryParams.enabled === "" || item.enabled === queryParams.enabled)
    );
  })
);
...
function formatAlgorithm(value: RateLimitAlgorithm) {
  return algorithmOptions.find((item) => item.value === value)?.label ?? value;
}
...
function toForm(scene: RateLimitSceneItem): RateLimitSceneForm {
  return {
    ...scene,
    dimensions: (scene.dimensions?.split(",").filter(Boolean) as RateLimitDimension[]) || [],
  };
}
function toPayload(): RateLimitScenePayload {
  return { ...formData, dimensions: formData.dimensions.join(",") };
}
function fetchData() {
  loading.value = true;
...
function handleOpenDialog(sceneCode?: string) {
  ...
  dialog.title = sceneCode ? "修改限次场景" : "新增限次场景";
  if (sceneCode)
    RateLimitSceneAPI.getFormData(sceneCode).then((data) => Object.assign(formData, toForm(data)));
}
...
function handleSubmit() {
  dataFormRef.value.validate((valid: boolean) => {
    ...
    const request = dialog.editing
      ? RateLimitSceneAPI.update(dialog.sceneCode, toPayload())
      : RateLimitSceneAPI.create(toPayload());
    request.then(() => {
...
function handleDelete(sceneCode: string) {
  ElMessageBox.confirm(`确认删除限次场景「${sceneCode}」吗？`, "警告", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  }).then(() =>
    RateLimitSceneAPI.delete(sceneCode).then(() => {
