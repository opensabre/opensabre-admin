    ...
    <el-tabs v-model="activeTab" class="release-tabs">
      <el-tab-pane label="待发布变更" name="pending">
        <el-card shadow="never">
          ...
          <el-alert
            title="正式发布是一次完整配置原子替换，会包含当前全部待发布变更。"
            type="warning"
            :closable="false"
            class="mb-5"
          />
          <el-alert
            v-if="candidate?.replacedLegacyRouteIds?.length"
            :title="`将原子替换 ${candidate.replacedLegacyRouteIds.length} 条遗留路由：${candidate.replacedLegacyRouteIds.join(', ')}`"
            type="info"
            :closable="false"
            class="mb-5"
            show-icon
          ...
          <el-descriptions v-loading="candidateLoading" :column="3" border>
            <el-descriptions-item label="当前配置版本">
              {{ currentVersion || "-" }}
            </el-descriptions-item>
            <el-descriptions-item label="API 路由">
              {{ candidate?.apiRouteCount ?? "-" }}
            </el-descriptions-item>
            <el-descriptions-item label="应用路由">
              {{ candidate?.applicationRouteCount ?? "-" }}
            </el-descriptions-item>
          </el-descriptions>
          <h4>编译后的路由预览</h4>
          <el-alert
            v-for="warning in candidateWarnings"
            :key="warning"
            :title="warning"
            type="warning"
            :closable="false"
          ...
          <el-table :data="candidate?.managedRoutes || []" border empty-text="本次候选没有托管路由">
            ...
            <el-table-column label="Predicates" min-width="300" show-overflow-tooltip>
              <template #default="{ row }">{{ definitionsText(row.predicates) }}</template>
            </el-table-column>
            <el-table-column label="Filters" min-width="300" show-overflow-tooltip>
              <template #default="{ row }">{{ definitionsText(row.filters) }}</template>
            </el-table-column>
          </el-table>
          <div class="mt-5 flex justify-end gap-2">
            <el-button :loading="candidateLoading" @click="validateCandidate">重新预检</el-button>
            <el-button type="primary" :loading="publishing" :disabled="!candidate" @click="publish">
              发布全部变更
            </el-button>
          </div>
        </el-card>
      ...
      <template v-if="detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="状态">
            <DictTag v-model="detail.release.status" code="gateway_release_status" />
          </el-descriptions-item>
          <el-descriptions-item label="目标版本">
            {{ detail.release.targetVersion || "-" }}
          </el-descriptions-item>
          <el-descriptions-item label="失败原因">
            {{ detail.release.failureReason || "-" }}
          </el-descriptions-item>
        </el-descriptions>
        <h4>变更项</h4>
        <el-table :data="detail.items" size="small">
          ...
          <el-table-column prop="itemId" label="对象" min-width="180" />
          <el-table-column prop="summary" label="摘要" min-width="260" />
        </el-table>
        <h4>实例加载确认</h4>
        <el-table :data="detail.instances" size="small">
          <el-table-column prop="instanceId" label="实例" min-width="180" />
          <el-table-column prop="loadedVersion" label="加载版本" min-width="180" />
          <el-table-column prop="status" label="状态" width="130" />
          <el-table-column prop="errorMessage" label="异常" min-width="220" />
        </el-table>
        <h4>路由探测</h4>
        <el-table :data="detail.routeProbes" size="small">
          <el-table-column prop="instanceId" label="实例" min-width="180" />
          <el-table-column prop="status" label="状态" width="130" />
          <el-table-column prop="missingRouteIdsJson" label="缺失路由" min-width="240" />
          <el-table-column prop="errorMessage" label="异常" min-width="220" />
        </el-table>
      </template>
      <template #footer>
        <el-button
          :loading="verifying"
          :disabled="detail?.release.status !== 'PARTIALLY_APPLIED'"
          @click="verifyInstances"
        >
          重新确认实例
        </el-button>
        <el-button
          ...
          plain
          :loading="rollingBack"
          :disabled="!detail?.release.targetVersion"
          @click="rollback"
        >
          回滚到此版本
        </el-button>
...
const { labelOf: releaseStatusLabel } = useDict("gateway_release_status");
...
async function loadCandidate() {
  ...
  try {
    ...
    candidate.value = await GatewayApiRouteAPI.validateRelease(current.version);
  } finally {
...
async function validateCandidate() {
  await loadCandidate();
  ElMessage.success("发布预检通过");
}
async function publish() {
  await ElMessageBox.confirm("将发布当前全部待生效变更，是否继续？", "统一发布确认", {
    type: "warning",
  });
  publishing.value = true;
  try {
    const result = await GatewayApiRouteAPI.publishRelease(currentVersion.value);
    ElMessage.success(`发布已提交：${releaseStatusLabel(result.status) || result.status}`);
    await Promise.all([loadCandidate(), loadHistory()]);
...
async function loadHistory() {
  ...
  try {
    releases.value = await GatewayApiRouteAPI.listReleases();
  } finally {
    historyLoading.value = false;
  }
}
async function openDetail(id: string) {
  detail.value = await GatewayApiRouteAPI.getRelease(id);
...
async function verifyInstances() {
  ...
  try {
    await GatewayApiRouteAPI.verifyReleaseInstances(detail.value.release.id);
    await openDetail(detail.value.release.id);
    ElMessage.success("实例加载状态已更新");
  } finally {
...
async function rollback() {
  if (!detail.value) return;
  await ElMessageBox.confirm("回滚会以该历史版本创建一次新的正式发布，是否继续？", "版本回滚", {
    type: "warning",
  });
  rollingBack.value = true;
  try {
    const current = await GatewayApiRouteAPI.getCurrentConfig();
    await GatewayApiRouteAPI.rollbackRelease(detail.value.release.id, current.version);
    detailVisible.value = false;
    await loadHistory();
    ElMessage.success("回滚发布已提交");
  } finally {
...
function definitionsText(items?: GatewayRuntimeRouteDefinition[]) {
  return (
    (items || [])
      .map(
        (item) =>
          `${item.name}(${Object.entries(item.args || {})
